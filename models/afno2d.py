...
class AFNO2DBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        o1_real = F.relu(
            torch.einsum('bifhw,io->bofhw', x_reshaped.real, self.w1[0]) -
            torch.einsum('bifhw,io->bofhw', x_reshaped.imag, self.w1[1]) +
            self.b1[0].unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
        )
        o1_imag = F.relu(
            torch.einsum('bifhw,io->bofhw', x_reshaped.imag, self.w1[0]) +
            torch.einsum('bifhw,io->bofhw', x_reshaped.real, self.w1[1]) +
            self.b1[1].unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
        )
        o2_real = (
            torch.einsum('bifhw,io->bofhw', o1_real, self.w2[0]) -
            torch.einsum('bifhw,io->bofhw', o1_imag, self.w2[1]) +
            self.b2[0].unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
        )
        o2_imag = (
            torch.einsum('bifhw,io->bofhw', o1_imag, self.w2[0]) +
            torch.einsum('bifhw,io->bofhw', o1_real, self.w2[1]) +
            self.b2[1].unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
        )
        ...
        x_out[:, :, :kept_h, :kept_w] = x_processed
