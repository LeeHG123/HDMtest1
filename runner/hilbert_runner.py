...
class HilbertDiffusion(object):
    ...
    def validate(self, model, val_loader, tb_logger, step):
        ...
        with torch.no_grad():
            for i, (x, y) in enumerate(val_loader):
                ...
                if self.is_2d:                          # (B,H,W,2)/(B,H,W,1) →  (B,H,W,2)/(B,H,W)
                    x_coord = x.to(self.device)         # 좌표는 마지막 dim=2 그대로
                    y_norm  = y.to(self.device).squeeze(-1)
                    x_coord_norm = x_coord / 10.0       # 정규화
                else:
    ...
    def train(self):
        ...
        for epoch in range(config.training.n_epochs):
            ...
            for i, (x, y) in enumerate(train_loader):
                ...
                if self.is_2d:
                    loss = hilbert_sobolev_loss_fn_2d(
                        model, self.sde, y_norm, t, e, x_coord_norm,
                        sobolev_weight=self.config.training.sobolev_weight,
                    )
                else:
    ...
    def visualize(self, x_true, x_pred, save_path):
        if self.is_2d:
            # ≈ matplotlib two‑panel heat‑map
            fig, axes = plt.subplots(1, 2, figsize=(6,3))
            im0 = axes[0].imshow(x_true.cpu(), origin='lower'); axes[0].set_title("GT")
            im1 = axes[1].imshow(x_pred.cpu(), origin='lower'); axes[1].set_title("Generated")
            fig.colorbar(im1, ax=axes.ravel().tolist(), shrink=0.6)
        else:
            plt.plot(x_true.cpu(), 'r-', label='GT')
            plt.plot(x_pred.cpu(), 'b--', label='Generated')
            plt.legend()
    ...
    def sample(self, score_model=None):
        ...
        # ──── ①-A UniPC 결과 시각화 : SRK 스타일(2-패널) ────
        if self.args.sample_type == "unipc_ode" and config.data.dataset in ["Gaussian" "Gaussian2D"]:
            ...
            # 2) power 계산
            y_pow_flat = y_plot.reshape(y_plot.size(0), -1)   # 또는 .view(...)
            y_gt_flat  = y00.reshape(y00.size(0), -1)         # GT likewise
            n_tests = y_pow_flat.size(0) // 10
            power_res = calculate_ci(y_pow_flat, y_gt_flat, n_tests=n_tests)
            print(f"[UniPC-log] power(avg 30) = {power_res}")

            # 3) 그림
            self.visualize(
                x_true=y_0[0] if not self.is_2d else y_0[0],   # GT
                x_pred=y_plot[0] if not self.is_2d else y_plot[0],
                save_path="visualization_unipc.png",
            )     
        ...
        if self.args.sample_type == "unipc_ode" and config.data.dataset in ["Gaussian" "Gaussian2D"]:
            ...
            self.visualize(
                x_true=y_0[0] if not self.is_2d else y_0[0],   # GT
                x_pred=y_plot[0] if not self.is_2d else y_plot[0],
                save_path="visualization_dpmpp.png",
            )      
        ...
        if self.args.sample_type == "unipc_ode" and config.data.dataset in ["Gaussian" "Gaussian2D"]:
            ...
            self.visualize(
                x_true=y_0[0] if not self.is_2d else y_0[0],   # GT
                x_pred=y_plot[0] if not self.is_2d else y_plot[0],
                save_path="visualization_deis.png",
            )     
        ...
        if self.args.sample_type == "unipc_ode" and config.data.dataset in ["Gaussian" "Gaussian2D"]:
            ...
            self.visualize(
                x_true=y_0[0] if not self.is_2d else y_0[0],   # GT
                x_pred=y_plot[0] if not self.is_2d else y_plot[0],
                save_path="visualization_tsit5.png",
            )     
